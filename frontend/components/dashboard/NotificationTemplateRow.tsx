...
import { updateNotificationTemplate } from "@/lib/api/templates";
...
export function NotificationTemplateRow({
  ...
  onUpdated: (updated: NotificationTemplate) => void;
}) {
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-foreground/50">
            {template.channel} · {template.locale} · {template.is_active ? "active" : "inactive"}
          </p>
        </div>
        <button
          type="button"
